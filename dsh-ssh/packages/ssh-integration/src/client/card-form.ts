/**
 * Staged form model for the ssh-remote card.
 *
 * This is a same-shape implementation of the official `CardForm` from
 * `@deepseek-ai/dsh-client-ui-settings-plugins` (the workspace pins
 * 0.1.5-rc.2): that package exports the *types* but not the class value, and
 * an external plugin cannot reach into another package's internals. The
 * behavior, field contracts and action semantics mirror the official form so
 * the card behaves identically to the built-in plugin cards.
 *
 ...
 * asked for; staged text makes what is on screen exactly what a save would
 * store. A field shows its effective value — the user layer over the
 * composition layer over the schema default — and whether the user layer
 * carries it.
 *
 * @module @local/dsh-ssh-integration/client/card-form
 */
...
/**
 * A whole-number field bounded to an inclusive range. Drafts outside the
 * range block the save rather than clamping silently.
 * @param field - field name inside the namespace section.
 * @param min - smallest accepted value.
 * @param max - largest accepted value.
 * @returns the field's conversion spec.
 */
export function rangedIntField(field: string, min: number, max: number): CardFieldSpec {
  return {
    field,
    format: value => typeof value === 'number' ? String(value) : '',
    parse: (text) => {
...
/**
 * A non-negative whole-number field.
 * @param field - field name inside the namespace section.
 * @returns the field's conversion spec.
 */
export function nonNegativeIntField(field: string): CardFieldSpec {
  return rangedIntField(field, 0, Number.MAX_SAFE_INTEGER)
}

/**
 * An absolute POSIX path field. A relative draft blocks the save: the remote
 * working directory is interpreted by the server, and a relative one would
 * silently mean a different directory per process.
 * @param field - field name inside the namespace section.
 * @returns the field's conversion spec.
 */
export function absolutePathField(field: string): CardFieldSpec {
  return {
    field,
    format: value => typeof value === 'string' ? value : '',
    parse: (text) => {
...
/**
 * An enumeration field over a fixed set of accepted spellings.
 * @param field - field name inside the namespace section.
 * @param values - the accepted spellings.
 * @returns the field's conversion spec.
 */
export function enumField(field: string, values: readonly string[]): CardFieldSpec {
  ...
  return {
    ...
    parse: (text) => {
      const trimmed = text.trim()
      if (trimmed === '') return { kind: 'clear' }
      if (!accepted.has(trimmed)) return undefined
      return { kind: 'set', value: trimmed }
    },
  }
}
